...
Const SqlPedido =
      ...
      '					 Ro.Descricao Rota, 1 as ArmazemId, Ped.Status, Ped.uuid, De.ProcessoId, De.Descricao processo into #Ped  '+sLineBreak+
...
Const RestSqlPedido = 'Drop table if exists #Pedidos'+sLineBreak+
      'Select Ped.PedidoId,	Ped.OperacaoTipoId,	Ped.OperacaoTipo,	Ped.Operacao,	Ped.Pessoaid,	Ped.CodPessoaERP,	Ped.Razao,	Ped.Fantasia,	Ped.DocumentoOriginal,	'+sLineBreak+
      '       Ped.DocumentoNr, Ped.DocumentoData,	Ped.RegistroERP,	Ped.DtInclusao, Ped.HrInclusao,	Ped.RotaId,	Ped.Rota,	Ped.ArmazemId,	Ped.Status,	'+sLineBreak+
      '       Ped.uuid,	PNF.NotaFiscal as NotaFiscalERP, Ped.ProcessoId, Ped.processo, Ped.Processo Etapa, Ped.DtProcesso,' + sLineBreak +
      ...
      'if object_id ('+#39+'tempdb..#lotes_x3'+#39+') is not null drop table #lotes_x3'+sLineBreak+
...
//      'Left Join vDocumentoEtapas DE On De.Documento = Ped.uuid' + sLineBreak +
 ...
 Const
    SqlPedidoParaProcessamento = 'Declare @PedidoId Integer    = :pPedidoId' +
      ...
      '	   Ped.ProcessoEtapa as ProcessoEtapa, Vs.StatusMin, Vs.StatusMax, Iv.QtdProdutos, Iv.Peso, Iv.Volume'+sLineBreak+
      'into #Pedidos'+sLineBreak+
...
Const SqlPedidoPrintTag = 'Declare @PedidoId Integer = :pPedidoId'+sLineBreak+
      ...
      'Drop table if exists #PedidoVolume'+sLineBreak+
...
//      '  And (De.Horario = (Select Max(Horario) From vDocumentoEtapas where Documento = Vlm.uuid and Status = 1))'+sLineBreak+
      ...
      '  And (@RotaId = 0 or Ped.RotaId >= @RotaId) and (@RotaId = 0 or Ped.RotaId <= @RotaIdFinal)'+sLineBreak+
  ...
  Const
    SqlPedidoPrintTagOLD = 'Declare @PedidoId Integer = :pPedidoId' + sLineBreak +
      'Declare @PedidoVolumeId Integer = :pPedidoVolumeId' + sLineBreak +
      'Declare @DataIni DateTime = :pDataIni' + sLineBreak +
      'Declare @DataFin DateTime = :pDataFin' + sLineBreak +
      'Declare @CodigoERP Integer = :pCodigoERP' + sLineBreak +
...
//      '													Inner Join vDocumentoEtapas De On De.Documento = Pv.uuid' + sLineBreak +
  ...
  Const
    SqlEstoqueCubagemCxaFechada = 'Declare @PedidoId Integer = :pPedidoId' + sLineBreak +
      ...
      '        PP.EmbalagemPadrao, PP.Quantidade / EmbSec as CxaFechada' + sLineBreak + 'From Pedido Ped' + sLineBreak +
      ...
      'WHERE Ped.OperacaoTipoId = 2 and (Prd.FatorConversao = PP.EmbalagemPadrao or PP.EmbalagemPadrao = 1) ' + sLineBreak +
...
Const SqlGerarVolumeLoteCaixaFechada = 'Declare @PedidoVolumeId Integer = (Select PedidoVolumeId From PedidoVolumes Where uuid = :pNewId)'+sLineBreak +
  ...
  Const
    SqlGetEstoqueEnderecoPorTipoDetalhes = 'Declare @EstoqueTipoId Integer = :pEstoqueTipoId'+sLineBreak +
      ...
      '       EET.Segregado, EET.Expedicao, Mov.Data DtUltimaMovimentacao, Ve.Mascara, Prd.Mascara mascarapicking, EEnt.dtInclusao, VE.Bloqueado' +sLineBreak +
      ...
      '             COALESCE ([6], 0) AS ' + #39 + 'Reserva' + #39 +', '+ sLineBreak+
      '             COALESCE ([7], 0) AS ' + #39 + 'QtdeTransfPicking' + #39 + sLineBreak+
      '      FROM (SELECT ProdutoId, EnderecoId, Est.LoteId, DescrLote, Rd.Data as Vencimento, Qtde, EstoqueTipoId' + sLineBreak +
      '            FROM Estoque Est' + sLineBreak +
      ...
      '			         From ReposicaoEstoqueTransferencia RET'+sLineBreak+
      '			         Inner Join vProdutoLotes Pl On Pl.LoteId = RET.LoteId'+sLineBreak+
      '            Where (@EstoqueTipoId in ( 0, 4 ) and RET.Qtde>0 ) ) AS Tbl' + sLineBreak +
...
//      'Left join vDocumentoEtapas DE On De.Documento = Vlm.Uuid' +sLineBreak +
      ...
      '		         Group by Vl.PedidoVolumeId, Tend.RuaId, TEnd.Rua, Tend.Zona) MS ON Ms.PedidoVolumeId = Vlm.PedidoVolumeId'+ sLineBreak +
      'Where --De.Horario = (Select Max(Horario) From vDocumentoEtapas where Documento = Vlm.uuid and Status = 1) and'+ sLineBreak +
      '      (@PedidoId = 0 or Vlm.PedidoId = @PedidoId) and (@PedidoVolumeId = 0 or Vlm.PedidoVolumeId = @PedidoVolumeId)'+sLineBreak +
...
Const SqlVolume = 'Declare @PedidoId Int  = :pPedidoId' + sLineBreak +
      ...
      '           , PEd.DocumentoNr, Rd.Data As DocumentoData, DePed.ProcessoId ProcessoIdPed' + sLineBreak +
      '           , P.PessoaId, P.Razao, P.CodPessoaERP, P.Fantasia' + sLineBreak +
...
//      'Left join vDocumentoEtapas DE On De.Documento = Vlm.Uuid' + sLineBreak +
  ...
  Const
    SqlGetVolumeConsulta = 'Declare @Datainicial DateTime    = :pDataInicial' +sLineBreak +
      'Declare @DataFinal DateTime      = :pDataFinal' + sLineBreak+
...
//      'Left join vDocumentoEtapas DePed On DePed.Documento = Ped.Uuid'+sLineBreak+
      ...
      '       PredioInicial, IsNull(Cc.CargaId, 0) CargaId, Pv.ProcessoIdPed'+sLineBreak+
...
Const SqlSaidaIntegracaoRetornoPedido = 'Declare @Pedido VarChar(36) = :pPedido' +
      sLineBreak +
  ...
  Const
    SqlSaidaIntegracaoRetornoVolume = 'Declare @PedidoId Integer = :pPedidoId' +
      ...
      sLineBreak +
      'Where PedidoId = @PedidoId and De.ProcessoId <> 15 and De.ProcessoId = (Select Max(ProcessoId) '
      + sLineBreak +
  ...
  Const SqlEtiquetaVolumePorRua1 =
      ...
      'Drop table if exists #CxaFechada'+sLineBreak+
      ...
      '	   De.ProcessoId, De.Descricao ProcessoEtapa, Pr.Ordem, TotalVolumes Into #PedidoVolume'+sLineBreak+
      ' '+sLineBreak+
      'From PedidoVolumes Pv'+sLineBreak+
      ...
      //'     And Pv.PedidoId in ('+sLineBreak+
      '--Substituir pelos numeros do pedidos )'+sLineBreak+
      ''+sLineBreak;

Const SqlEtiquetaVolumePorRua2 =
      'select Pv.PedidoVolumeId, Vl.Itens, Vl.QtdSuprida, Vl.Inicio, Vl.Termino, Vl.Mascara Into #VL'+sLineBreak+
      ...
      ''+sLineBreak+
      'select Pv.PedidoVolumeId, Z.ZonaId, Z.Zona Into #Zona'+sLineBreak+
      'From #PedidoVolume Pv'+sLineBreak+
      ...
      ''+sLineBreak+
      'select Pv.PedidoVolumeId, VCxaFechada.CodProduto, VCxaFechada.Descricao, VCxaFechada.Picking, VCxaFechada.Lote, VCxaFechada.Vencimento Into #CxaFechada'+sLineBreak+
      'From #PedidoVolume Pv'+sLineBreak+
      'Inner Join (Select Vl.PedidoVolumeId, Prd.CodProduto, Prd.Descricao, Prd.EnderecoDescricao Picking, Pl.DescrLote Lote, Dv.Data Vencimento'+sLineBreak+
      '           From PedidoVolumeLotes Vl'+sLineBreak+
      '		         Inner Join ProdutoLotes Pl On Pl.LoteId = Vl.LoteId'+sLineBreak+
      '		         Inner join vProduto Prd On Prd.IdProduto = Pl.ProdutoId'+sLineBreak+
      '		         Inner Join Rhema_Data DV On DV.IdData = Pl.Vencimento'+sLineBreak+
      '		         ) VCxaFechada ON VCxaFechada.PedidoVolumeId = Pv.PedidoVolumeId'+sLineBreak+
      'Where Pv.EmbalagemId Is Null'+sLineBreak+
      ''+sLineBreak+
      'Select Ped.* , Vl.Itens, VL.QtdSuprida, VL.Inicio, VL.Termino, Vl.Mascara, Z.Zona, Coalesce(Ped.EmbalagemId, 0) EmbalagemId,'+sLineBreak+
      ...
      'Left Join #Zona Z ON Z.PedidoVolumeId = Ped.PedidoVolumeId'+sLineBreak+
...
Const SqlEtiquetaPorVolume_120524 = 'SET STATISTICS IO OFF' + sLineBreak +
  ...
  Const
    SqlIdentificaVolumeCxaFechada = 'SET STATISTICS IO OFF' + sLineBreak +
      ...
      'Declare @PedidoVolumeId Integer = :pPedidoVolumeId' + sLineBreak +
      'Select Pv.PedidoId, Pd.documentooriginal, Pv.PedidoVolumeId, Pv.Sequencia,Pe.CodPessoaERP, Pe.Razao, Pe.Fantasia, '+sLineBreak +
      '       Ro.RotaId, Ro.Descricao Rotas,' + sLineBreak +
      ...
      '       De.ProcessoId, De.Descricao ProcessoEtapa, TotalVolumes' + sLineBreak +
      'From PedidoVolumes Pv' + sLineBreak +
      'Inner join Pedido Pd On Pd.PedidoId = Pv.PedidoId' + sLineBreak +
      'Inner Join Pessoa Pe On Pe.PessoaId = Pd.PessoaId' + sLineBreak +
      'Inner Join RotaPessoas Pr on Pr.PessoaId = Pe.PessoaId' + sLineBreak +
      ...
      'Where Pv.PedidoVolumeId = @PedidoVolumeId' + sLineBreak +
      '      And DE.ProcessoId = (Select Max(ProcessoId) From vDocumentoEtapas where Documento = Pv.uuid and Status = 1)'+ sLineBreak +
      'Order by Pv.PedidoVolumeId, Tend.Descricao';
...
Const SqlPedidoCarga = 'Declare @PedidoId Integer = :pPedidoId' + sLineBreak +
      'Declare @DataIni DateTime = :pDataIni' + sLineBreak +
      'Declare @DataFin DateTime = :pDataFin' + sLineBreak +
      'Declare @PessoaId Integer = :pPessoaId' + sLineBreak +
      'Declare @Razao VarChar(100) = :pRazao' + sLineBreak +
      ...
      'IF OBJECT_ID('+#39+'tempdb..#PCub'+#39+') IS NOT NULL Drop Table #PCub'+sLineBreak+
      'IF OBJECT_ID('+#39+'tempdb..#PCub'+#39+') IS NOT NULL Drop Table #Etapa'+sLineBreak+
      ...
      '      (@PessoaId = 0 or P.PessoaId = @PessoaId) and'+sLineBreak+
      '				  (@Razao = '+#39+#39+' or P.Razao Like @Razao) and'+sLineBreak+
      '      (@RotaId = 0 or @RotaId = RP.RotaId) and (@OperacaoTipoId=0 or Ped.OperacaoTipoId = @OperacaoTipoId)'+sLineBreak+
      '      And ((@Cargaid = 0 and Cp.CargaId Is Null) or (@CargaId <> 0 and (Cp.CargaId Is Null or Cp.CargaId = @CargaId)))'+sLineBreak+
      '--#PCub'+sLineBreak+
      'select Pv.PedidoId, Cast(Sum(Pvl.QtdSuprida*Prd.PesoLiquido)/100 as decimal(15,3)) as Peso,'+sLineBreak+
      '       Sum(cast(Cast(Pvl.QtdSuprida*(Prd.Altura*Prd.Largura*Prd.Comprimento) as Decimal(15,6))/1000000 as Decimal(15,6))) Volm3'+sLineBreak+
      '       Into #PCub'+sLineBreak+
      'From PedidoVolumes PV'+sLineBreak+
      ...
      'Group by PV.PedidoId'+sLineBreak+
      '--#Etapa'+sLineBreak+
      'Drop Table If Exists #Etapa'+sLineBreak+
      'Select Pv.PedidoId, Sum((Case When EmbalagemId Is Null then 1 Else 0 End)) TCxaFechada,'+sLineBreak+
      '                    Sum((Case When EmbalagemId Is not Null then 1 Else 0 End)) TCxaFracionada,'+sLineBreak+
      '				    Sum((Case When De.ProcessoId = 15 then 1 Else 0 End)) Cancelado Into #Etapa'+sLineBreak+
      'From #Ped Ped'+sLineBreak+
      ...
      '     , Sum(PCub.Peso) Peso, Sum(PCub.Volm3) Volm3 Into #Pedidos'+sLineBreak+
      'From #Ped Ped'+sLineBreak+
      'Left Join vDocumentoEtapas DE On De.Documento = Ped.uuid'+sLineBreak+
      'Left Join #Etapa Etapa On Etapa.PedidoId = Ped.PedidoId'+sLineBreak+
      'Left Join #PP as PP On Pp.PedidoId = Ped.PedidoId'+sLineBreak+
      'Left Join #VL as VL On Vl.PedidoId = Ped.PedidoId'+sLineBreak+
      'Left Join #PCub PCub ON PCub.PedidoId = Ped.PedidoId'+sLineBreak+
      'where DE.ProcessoId = (Select Max(ProcessoId) From vDocumentoEtapas where Documento = Ped.uuid and Status = 1)'+sLineBreak+
      '       and Ped.OperacaoTipoId = 2 and (De.ProcessoId In (13, 14))'+sLineBreak+
...
Const SqlGetPedidoResumoAtendimento = 'Declare @PedidoId Integer = :pPedidoId'+sLineBreak +
      ...
      'Declare @DataInicial DateTime = :pDataInicial' + sLineBreak +
      'Declare @DataFinal DateTime   =  :pDataFinal' + sLineBreak +
      'select Ped.PedidoId, Rd.Data, PP.Produtoid, Prd.CodProduto, Descricao, EnderecoDescricao Picking, '+sLineBreak +
      '       Zonaid, ZonaDescricao, Sum(PP.Quantidade) Demanda,  '+sLineBreak+
      '       (Case When PP.EmbalagemPadrao = 1 then Prd.UnidadeSigla '+sLineBreak+
      '             Else '+#39+'Cxa c/ '+#39+'+Cast(PP.EmbalagemPadrao as Varchar) End) Embalagempadrao, '+sLineBreak+
      '       Sum(Coalesce(QtdSuprida, 0)) QtdSuprida, Prd.Volume*Sum(Coalesce(QtdSuprida, 0)) '+#39+'Volume(Cm3)'+#39+', '+sLineBreak+
      '       Cast(Prd.PesoLiquido*Sum(Coalesce(QtdSuprida, 0))/1000 as decimal(15,3)) Peso'+sLineBreak +
      'From PedidoProdutos PP' + sLineBreak +
      'Inner Join Pedido Ped On Ped.PedidoId = Pp.PedidoId' + sLineBreak +
      'inner Join Rhema_Data RD On Rd.IdData = Ped.DocumentoData' + sLineBreak +
      'Inner Join vProduto Prd On Prd.IdProduto = PP.ProdutoId' + sLineBreak +
      'Left Join (select Vlm.PedidoId, Pl.Produtoid, Sum(Vl.QtdSuprida) QtdSuprida'+sLineBreak+
      '           From PedidoVolumeLotes Vl' + sLineBreak +
      '           Inner Join PedidoVolumes Vlm On Vlm.PedidoVolumeId = Vl.PedidoVolumeId'+sLineBreak +
      '           Inner join ProdutoLotes Pl On Pl.LoteId = Vl.LoteId' +sLineBreak +
      '           Inner Join vDocumentoEtapas De on De.Documento = Vlm.Uuid' +sLineBreak +
      ...
      '                 And De.ProcessoId <> 15' + sLineBreak +
      '           Group by Vlm.PedidoId, Pl.Produtoid) As VolLote On VolLote.PedidoId = Pp.PedidoId and PP.ProdutoId = VolLote.ProdutoId'+sLineBreak +
      'Where (@PedidoId = 0 or Pp.PedidoId = @PedidoId)' +sLineBreak +
      '  and (@Divergencia = 0 or PP.Quantidade <> Coalesce(QtdSuprida, 0))'+sLineBreak +
      '	 and (@DataInicial = 0 or Rd.Data >= @DataInicial) and (@DataFinal = 0 or Rd.Data <= @DataFinal)'+sLineBreak +
      'Group by Ped.PedidoId, Rd.Data, PP.Produtoid, Prd.CodProduto, Descricao, EnderecoDescricao, Zonaid, ZonaDescricao, '+sLineBreak +
      '         Embalagempadrao, Prd.UnidadeSigla, Prd.UnidadeSecundariaSigla, Prd.FatorConversao, Prd.Volume, Prd.PesoLiquido'+sLineBreak +
      'order by descricao, Rd.Data, Pedidoid, ProdutoId';
...
Const SqlGetCargaPessoas =
      ...
      '       Pes.Fantasia, Rp.ordem, Conferido Into #CargaP'+sLineBreak+
      'From CargaPedidos Cp'+sLineBreak+
      ...
      '	      Sum((Case When Conferido = 1 then 1 Else 0 End)) Conferido'+sLineBreak+
      '	    , Sum(PedCub.Peso) Peso, Sum(PedCub.Volm3) Volm3'+sLineBreak+
      'From #CargaP CP'+sLineBreak+
...
Const SqlGetEvolucaoAtendimentoUnidZona =
      ...
      'Declare @RotaId Integer = :pRotaId' + sLineBreak +
      '' + sLineBreak +
      'Drop table if exists #Pedido'+sLineBreak+
      'Drop table if exists #PedProd'+sLineBreak+
      'Drop table if exists #PP'+sLineBreak+
      'Drop table if exists #CanceladoNProcessado'+sLineBreak+
      'Drop table if exists #VolumeLotes'+sLineBreak+
      'Drop table if exists #CorteAutomatico'+sLineBreak+
      'Drop table if exists #VolumeProcessado'+sLineBreak+
      ''+sLineBreak+
      ...
      'From #Pedido Ped'+sLineBreak+
      'Inner join (Select PedidoId, ZonaId, Quantidade'+sLineBreak+
      '            From PedidoProdutos PP'+sLineBreak+
      '            Inner join vProduto Prd On Prd.IdProduto = PP.ProdutoId'+sLineBreak+
      '            Where @ZonaId = 0 or Prd.ZonaId=@Zonaid) PP on PP.PedidoId = Ped.PedidoId'+sLineBreak+
      'Group by Ped.PedidoId, Ped.ProcessoId, PP.ZonaId'+sLineBreak+
      ''+sLineBreak+
      'select Pv.PedidoId, Pl.ZonaId, Sum(Vl.Quantidade) Quantidade Into #VolumeProcessado'+sLineBreak+
      'From PedidoVolumeLotes Vl'+sLineBreak+
      'Inner join PEdidoVOlumes Pv On Pv.PedidoVOlumeId = Vl.PedidoVolumeId'+sLineBreak+
      'Inner join #Pedido Ped On ped.PedidoId = Pv.PedidoId'+sLineBreak+
      'Inner join vProdutoLotes Pl on Pl.Loteid = vl.Loteid'+sLineBreak+
      ...
      ''+sLineBreak+
      'Select PP.ZonaId, PP.Demanda Demanda , sum(PP.Demanda-IsNull(Vl.Quantidade, 0)) Corte  Into #CorteAutomatico'+sLineBreak+
      'from #PedProd PP'+sLineBreak+
      ...
      ''+sLineBreak+
      'select pp.Pedidoid, Pp.Zonaid, sum(Pp.Demanda) CanceladoNProcessado Into #CanceladoNProcessado'+sLineBreak+
      'From #PedProd Pp'+sLineBreak+
      ...
      '	   Sum(Case When De.ProcessoId >= 13 and De.ProcessoId <> 15 then Vl.Quantidade Else 0 End) QtdExpedido,'+sLineBreak+
  ...
  Const
    SqlRelRecebimento = 'Declare @PedidoId Integer = :pPedidoId' + sLineBreak +
      ...
      sLineBreak +
      'where DE.ProcessoId = (Select Max(ProcessoId) From vDocumentoEtapas where Documento = Ped.uuid and Status = 1) and Ped.OperacaoTipoId = 3'
      + sLineBreak +
      '      and (@ProcessoId = 0  or De.ProcessoId = @ProcessoId)' + sLineBreak
  ...
  Const
    SqlDshCheckOut =
      ...
      + sLineBreak +
      '			               Where Dpv.ProcessoId = (Select Max(ProcessoId) From vDocumentoEtapas where Documento = Pv.uuid and Status = 1)'
      + sLineBreak +
    ...
    //  '      Left Join vDocumentoEtapas Pe7 On Pe7.Documento = Pv.Uuid and Pe7.ProcessoId = 7 And Pe7.Status = 1'+ sLineBreak +
    //  '      Left Join vDocumentoEtapas Pe8 On Pe8.Documento = Pv.Uuid and Pe8.ProcessoId = 8 And Pe8.Status = 1'+ sLineBreak +
      '      Inner Join Pedido Ped On Ped.Pedidoid = Pv.Pedidoid'+ sLineBreak +
      '      Inner Join Usuarios U On U.Usuarioid = De.UsuarioId'+ sLineBreak +
      '      Inner Join Rhema_Data Rd On Rd.IdData = Ped.DocumentoData' + sLineBreak +
  ...
  Const
    GetDesempenhoExpedicao =
      ...
      sLineBreak + 'WHERE (De.ProcessoId = (SELECT MAX(ProcessoId)' + sLineBreak +
      '                     FROM dbo.vDocumentoEtapas' + sLineBreak +
      '                     WHERE (Documento = PV.uuid) AND (Status = 1)))' +
