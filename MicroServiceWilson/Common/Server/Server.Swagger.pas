...
uses
  ...
  Server.AngularModel,
  Server.Utils.Strings,
  Swager.Json.Paths;

type
  TSwaggerGenerator = class
  private
    FoutPaht: string;
  ...
  public
    ...
    destructor destroy;
  published
    property SwaggerConfig: TSwaggerDto read FbodySwager write FbodySwager;
  end;
...
constructor TSwaggerGenerator.create;
begin
  FbodySwager := TSwaggerDto.create;
...
procedure TSwaggerGenerator.MakeReposioty;
var
  _index: integer;
...
destructor TSwaggerGenerator.destroy;
begin
  FbodySwager := TSwaggerDto.create;
  FreeAndNil(FOutJson);
...
procedure TSwaggerGenerator.GetClassesProc(AClass: TPersistentClass);
var
...
class function TSwaggerGenerator.GetRttiType(Obj: TObject): TRttiType;
var
  RttiContext: TRttiContext;
begin
  RttiContext := TRttiContext.create;
...
procedure TSwaggerGenerator.setSchemaProps(sFieldType, sDescription, sSize,
  sName, sContraints: string; pRequired: TJSONArray; pProperts: TjsonObject);
var
  Obj: TjsonObject;
...
procedure TSwaggerGenerator.ParseClass(AClassName: String);
var
  RttiContext: TRttiContext;
...
begin
  ...
  LResouce.Flags := LFlags;
  LResouce.CreateNodes();
  ...
  begin
    ...
    begin
      if (RttiAttribute is DBField) and
        ((RttiAttribute as DBField).TableSource = '') then
      begin
