...
begin
  ...
  sDiretorioAplicacao := ExtractFilePath(Application.ExeName)+'Log\';
  if Not DirectoryExists(sDiretorioAplicacao) then
     CreateDir(sDiretorioAplicacao);
  if Not DirectoryExists(ExtractFilePath(Application.ExeName)+'Relatorio\') then
     CreateDir(ExtractFilePath(Application.ExeName)+'Relatorio\');

  AssignFile(FArquivoLog, sDiretorioAplicacao + 'Log'+FormatDateTime('yyymmdd',Date())+'.txt');

  // se o arquivo não existir, é criado
  if not FileExists(sDiretorioAplicacao + 'Log'+FormatDateTime('yyymmdd',Date())+'.txt') then begin
     Rewrite(FArquivoLog);
