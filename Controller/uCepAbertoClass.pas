...
Uses System.Classes, Dialogs, Generics.Collections, System.SysUtils, UDmCep,
     Rest.Json, IdHTTP, System.StrUtils, System.Json;
...
begin
  With DmCep.QryPesquisar do Begin
    Close;
    Sql.Clear;
    Sql.Add('Select * From VCepPesquisa where Cep = '+QuotedStr(Cep));
    Open;
    if Not IsEmpty() then Begin
       Result := TCepAberto.Create;
       Result.FCep := FieldByName('Cep').AsString;
       Result.FLogradouro  := FieldByName('Log_Tipo_Logradouro').AsString+' '+FieldByName('Log_No').AsString;
       Result.FComplemento := FieldByName('Log_Complemento').AsString;
       Result.FIdBairro    := FieldByName('BAI_NU_SEQUENCIAL').AsInteger;
       Result.FBairro      := FieldByName('Bai_No').AsString;
       Result.FIbge        := FieldByName('MunCodDv').AsString;
       Result.FCidade      := FieldByName('MunNomEx').AsString;
       Result.FEstado      := FieldByName('Ufe_Sg').AsString;
       Result.Ddd          := StrToIntDef(FieldByname('Ddd').AsString, 0);
       close;
       Exit;
    End;
    Close;
  End;
  try
//    DmRhemaWMS.RESTClientCep.BaseURL := 'http://www.cepaberto.com/api/v3/nearest?lat='+pLatitude+'&lng='+pLongitude;
    DmuEvolut.RESTClientCep.BaseURL := 'http://www.cepaberto.com/api/v3/cep?cep='+Cep;
    DmuEvolut.RESTRequestCep.Execute;
    Result := JsonToClass(tJsonObject(DmuEvolut.RESTRequestCep.Response.JSONValue) as TJsonObject);
    if Result.Ibge <> '' then Begin
       AddMunicipio(Result.Ibge, Result.Cidade, Result.Estado, Result.Cep, Result.Latitude, Result.Longitude, Result.Altitude);
       AddCep(Result.Cep, Result.Ibge);
       if (Result.Bairro<>'') and (Result.Logradouro<>'') Then AddLogradouro(Result.Logradouro, Result.Estado, Result.Bairro, Result.Cidade, Result.Cep, Result.Complemento);
       If Result.DDD.ToString <> '' Then AddDDD(Result.Ibge, Result.Ddd);
    End;
    If Result.Bairro <> '' Then Result.IdBairro := SeekBairro('', Result.Bairro, Result.Estado, True);
  Except
      Result := Nil;
  end;
//  https://www.youtube.com/watch?v=6kecuVQrRCY
end;
...
begin
   ...
   try
     ...
     Result.Ddd         := TrataObjeto(LJSONObject, 'ddd').ToInteger();
