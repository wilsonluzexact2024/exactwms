...
{$Region Endereços}
  TEndereco = class
    ...
    public
      ...
      class procedure Excluir(pCodigo: Integer; pHistorico   : THistorico);
  End;
  TArrayListaEndereco = Array Of TEndereco;
  ...
  private
    ...
    Public
      Function Size : Integer;
      Function Add(Item : TEndereco) : Integer;
  End;
{$EndRegion}
...
Function TEndereco.GetEnderecoFormatado : String;
begin
  Result := FEndereco;
...
class function TEndereco.ListarEnderecamento(pCodigoSetor: string;
...
class function TEndereco.ListarEnderecos(pEndInicial, pEndFinal: string;
  pTipo: Integer): OleVariant;
begin

end;

class function TEndereco.PegarEndereco(pCodigo: Integer;
  pNome: String): TEndereco;
begin
end;

class function TEndereco.Pesquisar(pCdEndereco, pCdSetor: Integer; pNome,
...
class function TEndereco.PesquisarSetor(pCdEndereco, pSetor: Integer;
...
procedure TEndereco.SetEnderecoFormatado(const Value: String);
begin
  FEndereco := Value;
  if Value <> '' then
...
procedure TEndereco.setRua(const Value : TEnderecoRua);
begin
  FRua := Value;
end;

procedure TEndereco.setSetor(const Value: TEnderecoSetor);
begin
  FSetor := Value;
end;

procedure TEndereco.setTipo(const Value: TEnderecoTipo);
begin
  FTipo:= Value;
end;
...
function TListaEndereco.Add(Item: TEndereco): Integer;
begin
  SetLength(FItems, Size+1);
  FItems[High(FItems)] := Item;
  Result := High(FItems);
