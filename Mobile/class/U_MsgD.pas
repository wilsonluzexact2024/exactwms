...
Uses
   ...
   FMX.Ani, FMX.Layouts, System.UIConsts, FMX.Edit, FMX.Controls;
 ...
 Type
  ...
  public
      ...
      Property EditFocus    : TEdit Read FEditFocus Write FEditFocus;
      procedure ShowMsgD;
      procedure CloseMsgD;
      procedure Clear;
...
procedure TMsgD.DelayedSetFocus(control: TControl);
begin
...
begin
  Exit; // Evitar que o usuário click no retangulo do fundo e sair da rotina
  CloseMsgD;
...
begin
    ...
    if FFormMsg.Width > 400 then
       RectMsg.Width   := 400
    Else RectMsg.Width := FFormMsg.Width - 50;
    ...
    if FTypeInfo = tMsgDInformation then
     RectInformation.Fill.Color := StringToAlphaColor('#FF7599C5')  //Roxo
    Else
    if FTypeInfo = tMsgDWarning then
     RectInformation.Fill.Color := StringToAlphaColor('#FFFFFFB9')  //Amarelo
    Else
    if FTypeInfo = tMsgDanger then
     RectInformation.Fill.Color := StringToAlphaColor('#FFFF7878')  //Vermelho
    Else
    ...
    if Not Assigned(EditFocus) then
    EditFocus := TEdit.Create( RectInformation );
    EditFocus.Margins.Top   := 20;
    EditFocus.Margins.Left  := 20;
    EditFocus.Margins.Right := 0;
    EditFocus.Margins.Bottom:= 2;
    EditFocus.Parent   := RectInformation;
    EditFocus.Width := 100;
    EditFocus.Text := 'TESTE';
   ...
   // RectMsg.Width           := LabelText.Width - 60;
    ...
    RectMsg.Opacity := 1;
    DelayEdSetFocus(EditFocus);
    EditFocus.Visible := False;
